...
class DelivarymanController extends Controller
{
    ...
    public function acceptedOrderList(){
        $newList = Order_list::where('delivary_username', session()->get('username'))->get();
        $data =array();
        ...
        //return $data;
        return view('Users.Delivaryman.acceptedOrderList')->with('data', $data);      
    }
    ...
    public function delivaryPaid(Request $req){
        ...
        $order->delivary_completed_at = date('Y-m-d H:i:s');
        $order->save();
        return redirect()->back()->with(session()->flash('alert-success', 'Delivary paid successsfully'));
    }
}
